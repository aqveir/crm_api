...
trait UserAvailabilityAction
{
	...
	public function setUserAvailabilityStatus(int $userId, int $statusId, int $modifiedBy=0)
	{
		$objReturnValue=null;
		try {
			$query = config('omnicrm-class.class_model.user_availability')::updateOrCreate([
				'user_id' => $userId
				], [
					'status_id' => $statusId, 
	...
	public function getUserAvailabilityStatus(int $userId)
	{
		$objReturnValue=null;
		try {
			$query = config('omnicrm-class.class_model.user_availability')::where([
				'user_id' => $userId
				])->firstOrFail();
	...
	*/
	public function getOnlineUsers(int $orgId)
	{
		$objReturnValue=null;
		try {
			$query = config('omnicrm-class.class_model.user')::with(['availability','country']);
	...
	*/
	public function getUserOnlineStatusById(int $orgId, int $userId)
	{
		$objReturnValue=null;
		try {
			$query = config('omnicrm-class.class_model.user')::with(['availability']);
	...
	*/
	...
	{	
		$objReturnValue=null;
		try {
			$query = config('omnicrm-class.class_model.user')::where('id', $userId);
			$query = $query->where('org_id', $orgId);
			$query = $query->where('is_active', 1);
